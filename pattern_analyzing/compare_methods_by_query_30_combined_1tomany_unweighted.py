...
def average_method_metrics(method_data_list):
    """Average metrics across multiple methods for each query."""
    ...
    for query_id in all_query_ids:
        ...
        # For each metric, calculate the average across all methods that have this metric
        for metric in all_metrics:
            values = []
            for method_data in method_data_list:
            ...
            if values:  # Only calculate average if we have values
                avg_metrics[query_id][metric] = sum(values) / len(values)
    
    return avg_metrics

def calculate_method_differences(method1_data, compare_methods_data, selected_metrics=None):
    """Calculate average metric difference between method1 and the average of compare_methods for each query."""
    diff_by_query = {}
    ...
    for query_id in common_queries:
        ...
        diffs = [method1_metrics[metric] - compare_metrics[metric] for metric in common_metrics]
...
def main():
    ...
    compare_methods_combined_avg = average_method_metrics(compare_methods_combined_list)
    
    # Calculate differences between method1 and average of comparison methods for each query
    diff_by_query = calculate_method_differences(method1_combined, compare_methods_combined_avg, selected_metrics)
    ...
    print(f"\nMiniLM Scores for {domain} domain using top {len(top_queries)} queries where {method1} outperforms {compare_methods_str}{metric_desc} (winners highlighted):")
    print(minilm_styled_df.to_string())
    print(f"\nE5 Scores for {domain} domain using top {len(top_queries)} queries where {method1} outperforms {compare_methods_str}{metric_desc} (winners highlighted):")
...
'''
# Examples:
# Use all metrics, comparing against a single method:
...
# Use all metrics, comparing against multiple methods:
python pattern_analyzing/compare_methods_by_query_30_combined_1tomany.py --domain hotel --method1 eqr --compare_methods q2e q2d --top_n 100
python pattern_analyzing/compare_methods_by_query_30_combined_1tomany.py --domain restaurant --method1 eqr --compare_methods q2e q2d --top_n 100

# Use only specific metrics:
python pattern_analyzing/compare_methods_by_query_30_combined_1tomany.py --domain restaurant --method1 eqr --compare_methods q2e q2d --top_n 100 --metrics map_at10 recall_at10 rprecision
python pattern_analyzing/compare_methods_by_query_30_combined_1tomany.py --domain travel --method1 eqr --compare_methods q2d none --top_n 90 --metrics map_at10 recall_at10 map_at30
python pattern_analyzing/compare_methods_by_query_30_combined_1tomany.py --domain hotel --method1 eqr --compare_methods q2e none --top_n 100 --metrics map_at10 recall_at10 map_at30
'''